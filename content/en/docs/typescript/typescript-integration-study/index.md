...
![](auto-import.gif)
<!-- {{< img src="auto-import.gif" alt="Rectangle" caption="<em>Rectangle</em>" class="border-0" >}} -->
<!-- {{< img-simple src="auto-import.gif" alt="Square" class="border-0 rounded-circle" >}} -->
...
`yarn tsc --init`
{{< alert icon="💡" text="Note : if you got the following error : “gyp: No Xcode or CLT version detected (source)”, execute this command: sudo xcode-select --reset" />}}
...
{{< alert icon="💡" text="Despite Typescript compiler check also the typing, it is as fast as Babel. If it is still too slow, an incremental compilation can be activated." />}}
...
{{< alert icon="💡" text="An interesting tool made by Airbnb can convert all Javascript code into TypeScript : https://github.com/airbnb/ts-migrate" />}}
...
{{< alert icon="💡" text="To prevent TypeScript checking: // @ts-ignore ou // @ts-nocheck" />}}
